...
TitleHTML = 'The Reference Model for COVID-19 - The First Multi-Scale Ensemble Disease Model'
...
PresentationTitle = panel.panel('# The Reference Model for COVID-19 attempts to explain US data', width=Width, height=40, margin = (0,0,0,0))
...
The Reference Model for disease progression was initially a diabetes model using the approach of assembling models and validating them against different populations from clinical trials. 

The model performed simulation at the individual level while modeling entire populations using the MIcro-Simulation Tool (MIST) that employed High Performance Computing (HPC).

The Reference Model was transformed to model COVID-19 with the start of the epidemic. The model is now composed of multiple models that represent different phenomena such as models for: infectiousness, transmission, human response, mortality, and observation. Some of those models were calculated using at different scales including cell scale, organ scale, individual scale, and population scale. 

The Reference Model has therefore now reached the achievement of being the first known multi-scale ensemble model for COVID-19. This project is ongoing and this presentation will be updated for each venue.
...
There were 4 relative infectiousness curves used in the ensemble extracted from [28],[29],[30].  Another model from [45] was added and will be used in the future version of the model. 

All those models were made public in [31].
...
4. ***Temperature Effect*** - A multiplier to probability of transmission representing the effect of temperature in the state In April 2020 according to NOAA compared to 49.7833333333333 degrees Fahrenheit. 
...
Observation models represent how an observer see the true umbers reported by the model. They attempt to imitate the distortion in observed numbers such as:
...
The interesting element in this simulation is the elimination of transmission models that depend on temperature and preference of a lengthy infectiousness profile. 
"""), width=Width, height=None)
...
Section8_1 = panel.panel(FixReferences(RefDict,"""### Combined Infectiousness Model
...
Section8 = panel.Column(Section8_1, Figure_5, margin = (0,0,0,0))



Section9_1 = panel.panel(FixReferences(RefDict,"""### Combined Mortality Model
...
Section9_2 = panel.panel(FixReferences(RefDict,"""
...
Section9 = panel.Column( Section9_1, Figure_6, Section9_2, Figure_3, margin = (0,0,0,0))



Section10_1 = panel.panel(FixReferences(RefDict,"""### Conclusions

* There is slow progress in explaining COVID-19

* However, it is still unclear if it is possible to explain the phenomena like COVID-19 impact in a population in a period of 3 weeks using only initial conditions and mechanistic models. 
...
* A near ideal observer model does not seem to be dominant - possibly since the phenomenon is not linear

* Future work that speeds up computation can help better explore low probability events like death
...
### Acknowledgments: 
This work used the High-Performance Computing Environment provided by the MIDAS Coordination Center, supported by NIGMS grant 5U24GM132013 and the NIH STRIDES program. Thanks for Rescale cloud who provided additional cloud infrastructure. Thanks to Amazon AWS and Microsoft Azure that provided cloud credits for simulation.  Thanks to John Rice ,Jered Hodges, United Solutions, Jeff Pape, Boris and Halina Barhak (my parents), and Ronen Ozer for hosting a computation server. Thanks to the following model contributors:
Lucas Boettcher - infectiousness model and observation model,
Robin Thompson and William Hart - infectiousness model,
Alan Perelson - infectiousness models,
Sen Pei - allowed deriving infectiousness model from their computations,
Eric Forgoston - response models,
Filippo Castiglione - mortality model,
Thanks to the MSM viral pandemic working group that helped make the connections and support model development. 
Thanks to the COVID tracking project for providing a special license for using the data.
Additional thanks for helping locate and interpret Temperature data: David Trossman, Bruce Shapiro, Scott Stephens (NOAA) .
Thanks to all other sources of data that make it available easily online, including Apple for mobility data, US Census, Los Alamos National Lab. Thanks to Harel Dahari who made the connection to the MIDAS network and IMAG wiki for providing information to connect to Rescale. Thanks to Philipp Rudiger, James Bednar, and Jean-Luc Stevens for helping with HoloViz technologies that created this interactive paper. 
...
### Reproducibility:
Results presented in this work are archived in the file MIST_Ref_COVID19_Prelim_2022_06_09.zip for reproducibility purposes. MIST version 0.99.7.7 and python 2.7.18 with the Anaconda distribution were used for executing the simulation.
Visualization processing is archived in: ExplorationCOVID19_2022_06_09_10K_1Apr_21Dx10x51.zip. 
...
SectionSelectorTab = panel.layout.Tabs (
                                        ...
                                        ('Results', Section7),
                                        ('Results Infectiousness', Section8),
                                        ('Results Mortality', Section9),

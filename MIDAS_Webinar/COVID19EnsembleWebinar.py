...
SavedFileName = 'COVID19_Ensemble_MIDAS_Webinar.html'
PublishURL = 'https://jacob-barhak.github.io/'+SavedFileName
CodePublishURL = 'https://github.com/Jacob-Barhak/InteractivePapers/tree/master/MIDAS_Webinar'
QRCodeFileName = 'COVID19_Ensemble_MIDAS_Webinar.png'
...
PresentationTitle = panel.panel('# The Reference Model for COVID-19 - The First Multi-Scale Ensemble Disease Model', width=Width, height=40, margin = (0,0,0,0))
PresentationVenue = panel.panel('***MIDAS Webinar - 28 May 2021***', width=200, height=40, margin = (0,0,0,0))

PresentationAuthors = panel.panel("By: ***[Jacob Barhak](https://sites.google.com/view/jacob-barhak/home)***", width=900, height=40, margin = (0,0,0,0))
...
The Reference Model has therefore now reached the achievement of being the first known multi-scale ensemble model for COVID-19. This project is ongoing and glimpses to future developments will be provided. 
...
Section9_1 = panel.panel(FixReferences(RefDict,"""### Future Work
Reported positive infections and mortality are observed by the reporter and not necessarily match the reality. The observed number can be exaggerated or under-counted for different reasons. 

A new version of the model is now able to include different human interpretations from multiple experts to match observed and actual simulated infections and deaths. Those models are also optimized as part of the ensemble. 

The infectiousness model from [31] was added and one mortality model removed. 

Initial test results are shown below. ***However, those results are preliminary from a test simulation and only shown as an example of how things might change.***

The simulation that produced those results includes mock data and simulated in smaller scale of 1,000 individuals per state. To reach proper results a larger simulation is needed with proper human interpretations. 

The purple bars at the model mixture plot represent the influence of each one of the mock human interpreters. 

"""), width=700, height=None)

Figure_9 = ObjectInlineHTML(ResourceDir + '/CombinedPlotInterpretations.html', Width=1150, Height=620)

Section9 = panel.Column( Section9_1, Figure_9, margin = (0,0,0,0))
...
* Future work of adding human interpretation to the computational models is underway.
...
Future work results are archive in: MIST_Ref_COVID19_Small_2021_05_03.zip and visualization archived in ExplorationCOVID19_2021_05_03_1Kx5x51_regenerated_2021_05_21.zip . 
